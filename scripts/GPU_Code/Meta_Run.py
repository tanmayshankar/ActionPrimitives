...
# RUN FRCNN
command = [ 'python {0}/save_detections_gpu_args.py --data {1}D{2}/ --numf {3}',
			'python Video_CPM_args.py {1}D{2}/ {3}',
			'python {4}/Interpolate_Depth_Args.py {1}D{2} {3}']
...
for d in range(1,11):
	...
	for j in range(len(command)):
		p[j] = subprocess.Popen(command[j].format(FRCNN_DIR, IMG_DIR, d, number_images[d-1], INTERP_DIR),shell=True)
