...
# Defining Rollout variables.
		...
		self.number_segment_candidates = 30
	...
	def calc_normf_segmentation(self):

		force_norm = npy.linalg.norm(self.force_roll,axis=1)

		window = 7

		self.segmentation_indices = scimax(force_norm,npy.greater,order=window)
		with file("seg_indices.npy",'w') as outfile:
			npy.save(outfile,self.segmentation_indices)
	...
	# 	self.segmentation_candidates = npy.argsort(force_dot_prod)
	...
	# 	for i in range(1,len(self.segmentation_candidates)):
	...
	# 	threshold = 10
	# 	if (abs(seg_indices[0]-0)<threshold):
	# 		seg_indices[0] = 0
	# 	else:
	# 		seg_indices = npy.append(0,seg_indices)
	# 	if (abs(seg_indices[-1]-self.time_steps-1)<threshold):
	# 		seg_indices[-1] = self.time_steps-1
	# 	else:
	# 		seg_indices = npy.append(seg_indices,self.time_steps-1)
	...
	# 	self.segmentation_indices=seg_indices
	# 	# print("Segment Indices Converted:",seg_indices*(float(len(self.demo_pos))/self.time_steps))

	def linear_interpolate(self,pos):
		# smooth_pos = gaussian_filter1d(pos,3.5,axis=0,mode='nearest')
		tr = npy.linspace(0,len(pos)-1,len(pos))	
		tr_new = npy.linspace(0,len(pos)-1,self.time_steps+2)

		ipos = npy.zeros((self.time_steps+2,3))
		...
		self.demo_vel = npy.diff(ipos,axis=0)[:self.time_steps]
		self.demo_acc = npy.diff(ipos,axis=0,n=2)[:self.time_steps]
		self.demo_pos = ipos[:self.time_steps]
