...
for k in range(len(lh_seg_inds)-1):
	for t in range(lh_seg_inds[k],lh_seg_inds[k+1]):
		...
		cv2.putText(label,"Time: {0} Segment: {1} Next: {2}".format(t,k,lh_seg_inds[k+1]),(500,1000),cv2.FONT_ITALIC,2,(255,255,255),3)

		cv2.addWeighted(label,0.5,img,0.5,0,img)		
		cv2.imshow("Right Hand Frame",img)
		
		if (abs(lh_seg_inds[k]-t)<window)or(abs(lh_seg_inds[k+1]-t)<window):				
			margin = 0
