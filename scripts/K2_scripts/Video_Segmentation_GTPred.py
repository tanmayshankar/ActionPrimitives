...
lhgt = npy.load(os.path.join(FILE_DIR.format(video_index),"LH_Seg.npy"))
rhgt = npy.load(os.path.join(FILE_DIR.format(video_index),"LH_Seg.npy"))

seg_ind = npy.append(lh_seg_inds,rh_seg_inds)
seg_ind.sort()

gt_seg = npy.append(lhgt,rhgt)
gt_seg.sort()
...
for k in range(len(seg_ind)-1):
	for t in range(seg_ind[k],seg_ind[k+1]):
		...
		cv2.rectangle(gtlabel,(0,900),(1920,1080),(255*(k%2),0,255*((k+1)%2)),-1)				
		cv2.putText(gtlabel,"Time: {0} Segment: {1} Next: {2}".format(t,k,seg_ind[k+1]),(500,1000),cv2.FONT_ITALIC,2,(255,255,255),3)
